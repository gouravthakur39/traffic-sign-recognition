...
for i in range(num_classes) :
    path = "C:\\Users\\goura\\Downloads\\archive\\Train\\{0}/".format(i)
    print(path)
...
y_test=pd.read_csv("C:\\Users\\goura\\Downloads\\archive\\Test.csv")
...
for f in labels:
    image=cv2.imread("C:\\Users\\goura\\Downloads\\archive\\Test/"+f.replace('Test/', ''))
    image_from_array = Image.fromarray(image, 'RGB')
    size_image = image_from_array.resize((height, width))
    data.append(np.array(size_image))
